...
if st.button("🚀 Start Quantum Experiment", type="primary"):
    with st.spinner("Training Premium Hybrid Ensemble... This requires ~2 minutes..."):
        ...
        angle_results = train_model(angle_model, data['X_train'], data['X_full_train'], data['y_train'], data['X_test'], data['X_full_test'], data['y_test'], epochs=epochs, verbose=False, class_weights=class_weights)

        amplitude_model = AmplitudeEncodingModel(n_qubits=3)
        amplitude_results = train_model(amplitude_model, data['X_train'], data['X_full_train'], data['y_train'], data['X_test'], data['X_full_test'], data['y_test'], epochs=epochs, verbose=False, class_weights=class_weights)
...
if st.session_state.training_done and st.session_state.results:
    ...
    with col_g:
        fig = go.Figure(data=[
            go.Bar(name='Test Accuracy', x=['Angle Encoding', 'Amplitude Encoding'], y=[angle_acc, amp_acc], marker_color=['#1f77b4', '#ff7f0e'])
        ])
        fig.update_layout(title="Amplitude vs Angle Encoding Accuracy", yaxis_title="Accuracy (%)", template="plotly_white")
